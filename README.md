...
| 2 | Advanced Data Wrangling: Extracting ecological signals from noisy systems | Pivoting data frames (`pivot_longer()`, `pivot_wider()`), splitting a data frame column into multiple columns, combining multiple data frame columns into one column, wrangling strings and dates using the [`stringr`](https://stringr.tidyverse.org) and [`lubridate`](https://lubridate.tidyverse.org) packages, and how to handle missing values in data frames. This workshop also included an exercise where I cleaned and analysed 4 data files containing information on water quality and predatory fish species along the Ross River Estuary gradient in Queensland, Australia. | 
